...
class TableBattleEngine {
  ...
  cleanupVisibleBoard(player) {
    ...
    player.board.traps.forEach((trap) => {
    ...
    player.board.traps = stillAliveTraps;
  ...
  commitDefense(player, card) {
    ...
    player.armor += block;
    if (card.effect?.shieldNext) {
      player.statuses.shieldNext += card.effect.shieldNext;
    }
    ...
    this.pushLog(`${player.name} gains ${block} armor from ${card.name}.`);
    return { type: "defense", armorGained: block };
  }
  ...
  triggerTrap(defender, attacker, attackCard) {
    if (!defender.board.traps.length) return null;

    const trap = defender.board.traps.shift();
    ...
    return {
      ...
      players: this.players.map((player) => ({
        ...
        board: {
          ...
          traps: player.board.traps.map((card) => ({ ...card })),
